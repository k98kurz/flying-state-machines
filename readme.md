...
The code is organized into four classes: `FSM`, `Transition`, `AsyncFSM`, and
`AsyncTransition`.
...
- `AsyncFSM` (mirrors `FSM`; `input()` and `trigger()` are async)
  - `async input(self, event: Enum|str, data: Any = None) -> Enum|str:`
  - *(all other methods are synchronous and identical to `FSM`)*
- `AsyncTransition` (mirrors `Transition`; `trigger()` is async)
  - `async trigger(self, context: dict = None, data: Any = None) -> None:`
  - *(all other methods are synchronous and identical to `Transition`)*
</details>

To see the full documentation, read the
[dox.md](https://github.com/k98kurz/flying-state-machines/blob/v0.3.1/dox.md)
...
methods (`would`, `can`, hooks, `pack`/`unpack`, `touched`) remain synchronous
and work identically.

Hooks on async classes accept both sync and async callables — coroutines are
...
This is a simple library with 52 tests across two test files. To run the tests,
clone the repo and then run the following:

```bash
python tests/test_classes.py
python tests/test_asynclasses.py
```

Each file has a visual output test (`touched()`), which I suggest inspecting.
The async variant additionally validates that async event hooks and transition
hooks are handled correctly.
